...
int AddOrder(BENSCHILLIBOWL* bcb, Order* order) {
   pthread_mutex_lock(&(bcb->mutex)); 
    ...
    while (IsFull(bcb)) { 
        pthread_cond_wait(&(bcb->can_add_orders), &(bcb->mutex)); //the queue is full and max num of orders has been reached
    }
    ...
    // move on to next order and update queue
    bcb->next_order_number += 1; 
    bcb->current_size += 1;
    
    // broadcast that orders can be recieved 
